...
let placement: Placement = 'center top'
let delay: number = 0
let theme: Theme = 'dark'
let trigger: Trigger = 'hover'

const updateBinds = (element: HTMLElement) => {
  placement = (element.getAttribute('placement') as Placement) || 'center top'
  delay = parseInt(element.getAttribute('delay') || '0')
  theme = (element.getAttribute('theme') as Theme) || 'dark'
  trigger = (element.getAttribute('trigger') as Trigger) || 'hover'
}

const compareBinds = (element: HTMLElement) => {
  return (
    placement !== (element.getAttribute('placement') as Placement) ||
    delay !== parseInt(element.getAttribute('delay') || '0') ||
    theme !== (element.getAttribute('theme') as Theme) ||
    trigger !== (element.getAttribute('trigger') as Trigger)
  )
}

const generateTooltip = (element: HTMLElement, binding: DirectiveBinding) => {
...
const removeTooltip = (element: HTMLElement) => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const tooltip: Directive = {
  mounted(element: HTMLElement, binding: DirectiveBinding) {
    generateTooltip(element, binding)
  },
  updated(element: HTMLElement, binding: DirectiveBinding) {
    if (binding.value !== binding.oldValue || compareBinds(element)) {
